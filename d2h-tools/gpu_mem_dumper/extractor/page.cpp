 ...
 *
 ******************************************************************************/
Page::Page(MemDump &dump, std::uint64_t addr, TransType type, std::uint8_t flag) : Trans(dump, addr, type, flag)
{
}
...
void Page::printTrans(std::uint64_t virtAddr)
{
    ...
    std::cout << "\t\t\t\t\t" << std::dec << std::setw(3) << std::setfill(' ') << (virtAddr >> shBits & mask)
              << "-->" + sizeStr + "-Page@0x";
    ...
    std::cout << "\t|V:" << validFlag << "|AP:"
              << (aperture == APERTURE_VIDEO_LOCAL              ? "VL"
                  ...
                  : aperture == APERTURE_SYSTEM_MEMORY_COHERENT ? "SC"
                                                                : "SN")
              << "|VOL:" << volatileFlag << "|E:" << encryptedFlag << "|P:" << priviledgedFlag << "|RO:" << readOnlyFlag
